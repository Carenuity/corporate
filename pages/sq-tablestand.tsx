      ...
      <section className="pt-4">
        <div className="container">
          ...
          <p className="mn-4">
            {state === 'en' && (
              <>
                o enable cloud connectivity and remote monitoring, a separate
                SQ-Connector is required (not included). One SQ-Connector can
                support multiple SQ-Panels at the same time, making the system
                highly scalable and efficient.
              </>
            )}
            {state === 'de' && (
              <>
                Zur Anzeige und Fernüberwachung über die Cloud wird ein
                separater SQ-Connector benötigt (nicht im Lieferumfang
                enthalten). Ein SQ-Connector kann mehrere Panels gleichzeitig
                verbinden.
              </>
            )}
          </p>
          ...
          <h4 className="mn-4">
            {state === 'en' && <>App Support:</>}
            {state === 'de' && <>App-Unterstützung:</>}
          </h4>
          <p className="mn-4">
            {state === 'en' && (
              <>
                The free Carenuity App (available for iOS and Android) allows
                you to set up, monitor and configure the SQ-Panel.
                <a
                  href="https://carenuity.com/carenuity-app-guide/"
                  target="blank"
                >
                  <u>View the App Guide</u>
                </a>
              </>
            )}
            {state === 'de' && (
              <>
                Die kostenlose Carenuity App (iOS & Android) ermöglicht die
                Inbetriebnahme, Konfiguration und Anzeige der
                Luftqualitätsdaten.
                <a
                  href="https://carenuity.com/carenuity-app-guide/"
                  target="blank"
                >
                  <u>Zur App-Anleitung</u>
                </a>
              </>
            )}
          </p>
          <h4 className="mn-4">
            {state === 'en' && <>Typical Use Cases:</>}
            {state === 'de' && <>Typische Einsatzbereiche:</>}
          </h4>
          <p className="mn-4">
            {state === 'en' && (
              <>
                <strong>1. Patient Rooms & Care Environments</strong>
                <br />
                In hospital or eldercare settings, the SQ-TableStand allows the
                SQ-Panel to be placed securely on a bedside table or shelf
                without the need for wall installation. This enables continuous
                monitoring of air quality parameters such as CO₂, humidity, and
                temperature in patient rooms—conditions that can significantly
                impact recovery and comfort. Its ability to detect human
                presence (while ignoring pets or movement from non-human
                sources) helps caregivers ensure the space is properly occupied
                and ventilated.
              </>
            )}
            {state === 'de' && (
              <>
                <strong>1. Patientenzimmer und Pflegeumgebungen</strong>
                <br />
                In Krankenhäusern oder Pflegeeinrichtungen ermöglicht der
                SQ-TableStand die sichere Platzierung des SQ-Panels auf
                Nachttischen oder Regalen - ganz ohne Wandmontage. So kann die
                Luftqualität im Patientenzimmer durchgehend überwacht werden,
                einschließlich CO₂-Wert, Luftfeuchtigkeit und Temperatur -
                wichtige Faktoren für Komfort und Genesung. Die integrierte
                Präsenzerkennung, die zwischen Menschen und Tieren
                unterscheidet, unterstützt das Pflegepersonal dabei,
                sicherzustellen, dass Räume angemessen belegt und belüftet sind.
              </>
            )}
          </p>
          ...
          <p className="mn-4">
            {state === 'en' && (
              <>
                ...
                <br />
                In healthcare or corporate reception areas, the SQ-TableStand
                positions the SQ-Panel prominently on counters or desks to
                monitor indoor environmental conditions. This is particularly
                useful in shared spaces, where maintaining optimal air quality
                helps reduce the spread of airborne illnesses and improves
                cognitive performance. As a mobile station accessory, it
                provides flexibility—allowing staff to relocate the device to
                different rooms as needed without drilling or permanent
                installation.
              </>
            )}
            {state === 'de' && (
              <>
                <strong>
                  2. Schreibtische, Empfangstheken, mobile Stationen
                </strong>
                <br />
                An Empfangstheken in Kliniken oder Büros wird der SQ-TableStand
                genutzt, um das SQ-Panel gut sichtbar auf Tresen oder
                Schreibtischen zu platzieren und so die Umgebungsbedingungen
                kontinuierlich zu überwachen. Besonders in gemeinsam genutzten
                Räumen trägt dies zur Reduktion von luftübertragenen
                Krankheitserregern und zur Verbesserung der Konzentration bei.
                Als Teil einer mobilen Station lässt sich das Gerät flexibel in
                unterschiedliche Räume mitnehmen - ganz ohne Bohrungen oder
          ...
          <p className="mn-4">
            {state === 'en' && (
              <>
                ...
                <br />
                At trade fairs or internal training sessions, the SQ-TableStand
                enables Carenuity to showcase the SQ-Panel in temporary setups.
                Placed on podiums or demo tables, the stand provides a stable
                platform to demonstrate real-time environmental monitoring,
                making it easy to explain the features and benefits of the
                system. Its modern design also helps maintain a clean and
                professional look, which is essential in exhibition
                environments.
              </>
            )}
            {state === 'de' && (
              <>
                <strong>
                  3. Sch Schulungsräume, Messen, flexible Einsätze
                </strong>
                <br />
                Bei Messen oder Schulungen ermöglicht der SQ-TableStand eine
                stabile und professionelle Präsentation des SQ-Panels auf
                Tischen oder Podesten. Das Gerät kann so flexibel aufgestellt
                werden, um in Echtzeit die Luftqualität zu demonstrieren und die
                Funktionen anschaulich zu erklären. Das moderne Design des
                Ständers sorgt dabei für eine ästhetisch ansprechende
                Präsentation, was gerade in Ausstellungsumgebungen besonders
                wichtig ist.
              </>
            )}
          </p>
          ...
          <p className="mn-4">
            {state === 'en' && (
              <>
                <strong>4. Smart Homes and Private Monitoring</strong>
                <br />
                In a smart home, the SQ-TableStand allows residents to place the
                SQ-Panel on a kitchen shelf or a bedroom dresser to track CO₂
                levels, temperature, humidity, and more—all without modifying
                the walls. For renters or people living in temporary
                accommodations, this is especially valuable as it avoids damage
                to the property. The presence detection feature adds another
                layer of smart automation, allowing the system to activate
                alerts or ventilation only when people—not pets—are in the room.
              </>
            )}
            {state === 'de' && (
              <>
                <strong>4. Smart Homes und private Überwachung</strong>
                <br />
                Im Smart Home erlaubt der SQ-TableStand die flexible Platzierung
                des SQ-Panels auf Küchenregalen, Schreibtischen oder Kommoden,
                um die Luftqualität - etwa CO₂-Werte, Temperatur oder
                Luftfeuchtigkeit - zu überwachen. Besonders für Mietwohnungen
                oder temporäre Wohnsituationen ist dies ideal, da keine
                Wandmontage nötig ist. Die integrierte Präsenzerkennung bietet
                zusätzliche Automatisierungsmöglichkeiten, etwa zur Aktivierung
                von Lüftungssystemen, wenn sich Personen (nicht Haustiere) im
                Raum befinden.
              </>
            )}
          </p>
                  ...
                  >
                    ...
                    <div style={{ padding: '10px' }}>
                      <h6 className="text-success">
                        Carenuity SQ-Tabelstand - Mobile Table Stand for
                        SQ-Panel with USB-C
                      </h6>
                      <p>
                        The Carenuity SQ-Tablestand transforms the SQ-Panel into
                        a flexible, mobile air quality monitoring solution -
                        ideal for dynamic environments like care rooms,
                        workspaces...
                      </p>
                      <div className="row">
