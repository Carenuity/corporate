...
const strings: Record<Lang, {
  ...
  en: {
    ...
    serviceCards: [
      ...
      { title: '4) Quality Assurance', bullets: [
    ...
    pricingNote: 'Transparent packages based on boards/variants. Request a quote — we respond within 24 h.',
    contactTitle: 'Get started',
    contactLead: 'We\'re happy to advise and can ship a first shareable solution build on request.',
...
const LangSwitch: React.FC<{value: Lang; onChange: (l: Lang) => void}> = ({value, onChange}) => (
  <div className="lang" role="radiogroup" aria-label="language switch">
    {(['de','en'] as Lang[]).map(l => (
      <button key={l} onClick={() => onChange(l)} className="btn" aria-pressed={value===l}>
        {strings[l].langLabel}
      </button>
    ))}
  </div>
...
const SolutionBuilderAsAService: React.FC<Props> = ({defaultLang='de', primary, accent, mailto='mailto:info@example.com?subject=Solution%20Builder%20als%20Service'}) => {
    ...
    <div className="sb-wrapper">
      ...
      <div className="container">
        <header>
          ...
          <p>{t.heroSubtitle}</p>
          <div className="cta">
            <a className="btn primary" href={mailto}>{t.ctaPrimary}</a>
            <a className="btn" href="#process">{t.ctaSecondary}</a>
          </div>
        ...
        <div className="container">
          <Section title={t.whatIsTitle}>
            ...
            <div className="badges">
              {t.whatIsBullets.map((b, i) => <span key={i} className="badge">{b}</span>)}
            </div>
          </Section>
        </div>
      </div>
      ...
      <main className="container">
        ...
        <Section title={t.contactTitle}>
          <p>{t.contactLead}</p>
          <div className="cta">
            <a className="btn primary" href={mailto}>{t.contactPrimary}</a>
            <a className="btn" href="#">{t.contactSecondary}</a>
          </div>
        </Section>
      </main>

      <footer>
        <small>© {new Date().getFullYear()} Solution Builder as a Service</small>
      </footer>
    </div>
  );
};
